...
const MinecraftAvatar = ({
  ...
  const getAvatarUrl = () => {
    ...
    // Direct URL to Minotar as primary source
    switch (type) {
      case 'head':
        return `https://minotar.net/avatar/${finalUsername}/${size}.png`;
      case 'bust':
        return `https://visage.surgeplay.com/bust/${size}/${finalUsername}`;
      case 'full':
        return `https://visage.surgeplay.com/full/${size}/${finalUsername}`;
      default:
        return `https://minotar.net/avatar/${finalUsername}/${size}.png`;
    }
  };
  ...
  const handleError = (e) => {
    ...
    if (e.target.src.includes('minotar.net')) {
      console.log('Trying mc-heads fallback');
      e.target.src = `https://mc-heads.net/avatar/${fallbackUsername}/${size}`;

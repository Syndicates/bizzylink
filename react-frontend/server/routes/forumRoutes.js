...
// Import the Minecraft notification service
...
router.get('/topics/:topicId/threads', async (req, res) => {
  try {
    ...
    const pinnedThreads = await ForumThread.find({
      ...
      .sort({ createdAt: -1 })
      .lean()
      .populate('authorId', 'username role avatarUrl');
  ...
  try {
    ...
    if (pageNum === 1) {
      ...
      posts = await ForumPost.find({ threadId })
        ...
        .skip(skip)
        .limit(limitNum)
        .populate('authorId', 'username role avatarUrl createdAt postCount')
        .lean();
    } else {
...
// Create a new thread
router.post('/topics/:topicId/threads', auth, async (req, res) => {
  try {
    const { topicId } = req.params;
    ...
    const canPinLock = req.user.roles && 
...
router.post('/threads/:threadId/posts', auth, async (req, res) => {
  try {
    ...
    const isAdminOrMod = req.user.roles && 
      (req.user.roles.includes('admin') || req.user.roles.includes('moderator'));
...
// Edit a post (only author, moderators, or admins)
...
router.post('/threads/:threadId/pin', auth, requireModerator, async (req, res) => {
  try {
    const { threadId } = req.params;
...
router.post('/threads/:threadId/lock', auth, requireModerator, async (req, res) => {
  try {
    const { threadId } = req.params;
...
// Trigger in-game notification for a thread
router.post('/threads/:threadId/notify', auth, requireModerator, async (req, res) => {
  try {
    const { threadId } = req.params;
...
// Admin-only routes for managing forum structure

// Create a new category (admin only)
router.post('/admin/categories', auth, requireAdmin, async (req, res) => {
  try {
...
router.post('/admin/topics', auth, requireAdmin, async (req, res) => {
  try {
